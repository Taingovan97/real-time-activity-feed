...
STACK_NAME="activity-feed"
# Migrations run on host, so always use localhost (ports are exposed)
DATABASE_URL="postgres://postgres:postgres@localhost:5432/activity_feed?sslmode=disable"
...
ensure_dependency_services_running() {
    local postgres_running=$(is_container_running "activity-feed-postgres" && echo "yes" || echo "no")
    local redis_running=$(is_container_running "activity-feed-redis" && echo "yes" || echo "no")
...
start_prod_like_mode() {
    ...
    # Swarm uses activity_feed_prod network, dev uses activity_feed_dev network (no conflict)
