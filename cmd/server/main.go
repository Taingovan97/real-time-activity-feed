// Package main provides the entry point for the real-time activity feed server.
package main
...
import (
	...
	"real-time-activity-feed/internal/config"
	v1Auth "real-time-activity-feed/internal/module/auth/adapters/rest/v1"
	authApp "real-time-activity-feed/internal/module/auth/application"
	authJWT "real-time-activity-feed/internal/module/auth/infrastructure/jwt"
	authInfra "real-time-activity-feed/internal/module/auth/infrastructure/repository"
	...
	redisInfra "real-time-activity-feed/internal/shared/redis"
	"real-time-activity-feed/internal/shared/response"
	"real-time-activity-feed/spa"
...
func main() {
	...
	feedHandler := v1Feed.NewFeedHandler(feedUseCase, eventPublisherUseCase, l)

	// Setup router
	router := setupRouter(cfg, l, authUseCase, authHandler, feedHandler)
...
func setupRouter(
	...
	feedHandler *v1Feed.FeedHandler,
) *gin.Engine {
	...
	setupAPIRouter(router, l, authUseCase, authHandler, feedHandler)
...
func setupAPIRouter(
	...
	v1PublicGroup := v1Group.Group("")
	{
		...
		feedHandler.RegisterPublicRoutes(v1PublicGroup)
	}
	...
	v1ProtectedGroup.Use(authMiddleware.RequireAuth())
	{
		...
		// Protected feed routes (auth required)
		feedHandler.RegisterProtectedRoutes(v1ProtectedGroup)
	}
}
...
func setupSPARouter(router *gin.Engine) {
	...
	router.GET("/js/feed.js", func(c *gin.Context) {
		c.Data(http.StatusOK, "application/javascript", spa.FeedJS)
	})
