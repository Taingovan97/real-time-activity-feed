...
- **`docker/docker-compose.dev.yml`**: Contains only dependency services (PostgreSQL, Redis) for development
  ...
  - Uses `activity_feed_dev` network and `activity_feed_dev_*` volumes
  - Containers are only started if they don't already exist
  - Stack name: `activity_feed` (defined in compose file)
- **`docker/docker-compose.swarm.yml`**: Full stack configuration for Docker Swarm (production-like mode)
  ...
  - Uses `activity_feed_prod` network and `activity_feed_prod_*` volumes
  - App service uses service names (postgres/redis) from Swarm network
  - Stack name: `activity_feed` (deployed via `docker stack deploy`)
...
- **`migrations/dev/`**: Development-only seed data migrations (test users, sample events)
...
- **feed events**: Pre-populated sample events for all test users
