...
│       └── feed/            # Feed Module (event publish + feed)
│           ├── domain/            # Domain layer
│           ├── application/       # Application layer
│           ├── adapters/          # Adapters layer
│           └── infrastructure/    # Infrastructure layer
...
- **Interfaces**: Descriptive names ending with the role (e.g., `UserRepository`, `FeedRepository`)
- **Structs**: PascalCase (e.g., `FeedEvent`, `eventPublisherUseCase`)
- **Functions**: PascalCase for exported, camelCase for unexported
- **Constants**: PascalCase with descriptive names (e.g., `RedisFeedKey`)

### File Organization

**Naming Conventions**:
- **Domain entities**: One file per entity (e.g., `user.go`, `token.go`, `feed_event.go`)
- **Domain errors**: `errors.go` in domain layer
- **Repository interfaces**: Grouped in `application/repository.go` or split by concern
- **Use cases**: One file per use case (e.g., `auth_usecase.go`, `feed_usecase.go`, `event_publisher_usecase.go`)
...
func (r *Repo) GetFeed(...) ([]domain.FeedEvent, error) {
    // ... query with DTO
...
func (h *Handler) PublishEvent(c *gin.Context) {
    if err := h.useCase.PublishEvent(ctx, userID, req); err != nil {
...
func NewFeedUseCase(
    cacheRepo FeedCacheRepository,  // interface
    logger *logger.Logger,
) *FeedUseCase {                    // concrete struct
    return &FeedUseCase{...}
}
