...
build:
	...
	@docker build -f docker/Dockerfile -t activity-feed-app:latest .
...
clean:
	...
	@docker ps -a --filter "name=activity-feed" --format "{{.ID}}" | xargs -r docker rm -f 2>/dev/null || true
	@echo "Removing dev and prod volumes..."
	@docker volume rm activity_feed_dev_postgres_data activity_feed_dev_redis_data \
		activity_feed_prod_postgres_data activity_feed_prod_redis_data 2>/dev/null || true
