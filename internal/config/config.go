...
func Load() (*Config, error) {
	config := &Config{
		Server: ServerConfig{
			...
			// Increased timeouts for long-lived live-update connections.
			// ReadTimeout: time to read request headers before a WebSocket upgrade.
			ReadTimeout: getDurationEnv("SERVER_READ_TIMEOUT", 10*time.Minute),
			// WriteTimeout: time to write HTTP responses and handshake data.
		...
		Database: DatabaseConfig{
			...
			DBName:          getEnv("DB_NAME", "activity_feed"),
