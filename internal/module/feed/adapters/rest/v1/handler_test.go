...
func TestFeedHandler_GetFeed_WithCombinedFilterAndSearch(t *testing.T) {
	gin.SetMode(gin.TestMode)
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	mockFeed := mocks.NewMockFeedUseCase(ctrl)
	mockEvents := mocks.NewMockEventPublisherUseCase(ctrl)
	mockFeed.EXPECT().GetFeed(gomock.Any(), domain.EventTypeUpload, "report", int64(10), int64(0)).Return([]domain.FeedEvent{{
		ID: "evt-1", UserID: "user-1", Username: "alice", EventType: domain.EventTypeUpload, Content: "uploaded report", CreatedAt: time.Now(),
	}}, int64(1), nil)

	w := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(w)
	c.Request = httptest.NewRequest(http.MethodGet, "/feed?limit=10&offset=0&event_type=upload&query=report", nil)
