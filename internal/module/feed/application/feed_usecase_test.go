...
func TestFeedUseCase_GetFeed_WithCombinedFilterAndSearch(t *testing.T) {
	ctx := context.Background()
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	persistence.EXPECT().GetFeed(ctx, domain.EventTypeUpload, "report", int64(10), int64(0)).Return(persisted, int64(1), nil)

	uc := NewFeedUseCase(persistence, users, broadcast, logger.New("info", false))
	entries, total, err := uc.GetFeed(ctx, domain.EventTypeUpload, "report", 10, 0)
